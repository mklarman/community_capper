module NflGameLogsHelper
	...
	def get_fav_and_dog
		...
		if @matchup.sport == "CFB"

			Team.all.each do |t|

				if t.matchup_abbr == @matchup.fav 

					@cfb_fav = t

				end

				if t.matchup_abbr == @matchup.dog 
	...
	def make_cfb_team_objects
		...
		Team.all.each do |t|
			...
			if t.sport == "CFB"
				...
				nfl_team[:team_name] = t.matchup_abbr
				nfl_team[:division] = t.div_or_conf
				nfl_team[:game_logs] = t.cfb_game_logs
